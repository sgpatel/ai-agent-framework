...
import {
    Card, Row, Col, Button, Modal, Form, Alert, Badge, Tab, Nav,
    Spinner, ProgressBar, Table, Dropdown, InputGroup, Toast, ToastContainer
...
export default function PluginManager() {
    ...
    const loadPluginData = async () => {
        ...
        try {
            ...
            setInstalledPlugins(installedRes.data || []);
            ...
            switch (action) {
                ...
                case 'deactivate':
                    ...
                    setInstalledPlugins(prev => prev.map(p =>
                        p.id === plugin.id ? { ...p, status: response.data.newStatus } : p
                    ));
    ...
    const savePluginConfig = async () => {
        ...
        try {
            ...
            setInstalledPlugins(prev => prev.map(p =>
                p.id === selectedPlugin.id ? { ...p, config: pluginConfig } : p
            ));
        ...
        <div className="plugin-manager p-4">
            ...
            <Row className="mb-4">
                <Col>
                    ...
                    <p className="text-muted">Manage and extend your AI Agent Framework with powerful plugins</p>
                </Col>
                <Col xs="auto">
                            ...
                            <Row>
                                {filteredInstalledPlugins.map(plugin => (
                                    <Col lg={6} xl={4} key={plugin.id} className="mb-4">
                                        <Card className={`h-100 ${plugin.status === 'active' ? 'border-success' : 'border-secondary'}`}>
                                            ...
                                            <Card.Body>
                                                ...
                                                <div className="mb-2">
                                                    <Badge bg="outline-primary" className="me-1">{plugin.category}</Badge>
                                                    <Badge bg="outline-secondary">{plugin.size}</Badge>
                                                </div>
                                                <div className="small text-muted mb-3">
                                                    <div>⭐ {plugin.rating} • 📥 {plugin.downloads.toLocaleString()}</div>
                                    ...
                                    <Card className="h-100">
                                        ...
                                        <Card.Body>
                                            ...
                                            <div className="small text-muted mb-2">
                                                <div>⭐ {plugin.rating} • 📥 {plugin.downloads.toLocaleString()}</div>
                                                <div>👤 {plugin.author}</div>
                                                <div className={plugin.compatibility.includes('Compatible') ? 'text-success' : 'text-warning'}>
                                ...
                                <Card>
                                    ...
                                    <Card.Body>
                                        <Alert variant="info">
                                            ...
                                            <p className="mb-0">Discover trending plugins that integrate seamlessly with your AI Agent Framework.</p>
                                        </Alert>
                ...
                <Modal.Body>
                    {selectedPlugin && (
                        <>
                            ...
                            <div className="mb-3">
                                <strong>Version:</strong> {selectedPlugin.version}<br />
                                <strong>Size:</strong> {selectedPlugin.size}<br />
                                <strong>Price:</strong> {selectedPlugin.price}
                            </div>
                            <Alert variant="info">
                                This plugin will be installed and activated immediately.
                            </Alert>
                        </>
                    )}
                </Modal.Body>
            ...
            <Modal show={showUploadModal} onHide={() => setShowUploadModal(false)}>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="primary" disabled={!uploadFile && !customPluginUrl} onClick={handleUploadPlugin}>
                        Upload Plugin
                    </Button>
                </Modal.Footer>
            </Modal>
