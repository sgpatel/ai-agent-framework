...
export default function StockAnalyzer() {
    ...
    const [watchlist, setWatchlist] = useState(['AAPL', 'GOOGL', 'MSFT', 'TSLA', 'AMZN', 'NVDA', 'META']);
    ...
    const loadStockData = async (stockSymbol) => {
        ...
        try {
            const [quoteRes, historyRes, indicatorsRes, signalRes, infoRes, marketRes, completeRes, comparisonRes] = await Promise.all([
    ...
    const shareAnalysisWithAgents = () => {
        if (completeAnalysis) {
            ...
            updateSharedData('financialMetrics', {
                symbol: symbol,
    ...
    const renderSignal = (signal) => {
        ...
        const variant = signal.signal === 'BUY' || signal.signal === 'STRONG_BUY' ? 'success' :
                ...
                <Row>
                    ...
                    <Col md={2}>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Select value={benchmarkSymbol} onChange={e => setBenchmarkSymbol(e.target.value)}>
            ...
            {realTimeQuote && (
                <Card className="mb-4">
                    <Card.Body>
                        <Row className="align-items-center">
                            <Col md={6}>
                                <h4 className="mb-1">
                                    ...
                                    <Badge
                                        bg={realTimeQuote.change >= 0 ? 'success' : 'danger'}
                                        className="ms-2"
                                    >
                                ...
                                <small className="text-muted">
                                    Last updated: {realTimeQuote.timestamp ? moment(realTimeQuote.timestamp).format('HH:mm:ss') : moment().format('HH:mm:ss')}
                                </small>
                            </Col>
                            <Col md={3}>
                                ...
                                {marketComparison && marketComparison.outperforming !== undefined && (
                                    <Badge
                                        ...
                                        className="ms-2"
                                    >
                                        {marketComparison.outperforming ? '📈 Outperforming' : '📉 Underperforming'} {benchmarkSymbol}
                                    </Badge>
                                )}
                            </Col>
                            <Col md={3} className="text-end">
                                <Button variant="outline-primary" size="sm" onClick={addToWatchlist} className="me-2">
                                    ⭐ Add to Watchlist
                                </Button>
                        ...
                        <Row className="mt-3">
                            <Col><small>Open: ${realTimeQuote.open?.toFixed(2)}</small></Col>
                            <Col><small>High: ${realTimeQuote.high?.toFixed(2)}</small></Col>
                            <Col><small>Low: ${realTimeQuote.low?.toFixed(2)}</small></Col>
                            <Col><small>Volume: {realTimeQuote.volume?.toLocaleString()}</small></Col>
                            <Col><small>Prev Close: ${realTimeQuote.previousClose?.toFixed(2)}</small></Col>
                        </Row>
                    </Card.Body>
                </Card>
            )}
                                    ...
                                    {companyInfo && (
                                        <Card>
                                            ...
                                            <Card.Body>
                                                ...
                                                <p><strong>Employees:</strong> {companyInfo.employees?.toLocaleString()}</p>
                                            ...
                                            <div className="card">
                                                <div className="card-body">
                                                    ...
                                                    <p className="card-text">
                                                        <Badge bg={pattern.signal === 'BULLISH' ? 'success' : pattern.signal === 'BEARISH' ? 'danger' : 'warning'}>
                                                            {pattern.signal}
                                                        </Badge>
                                                        <br />
                        ...
                        {marketComparison ? (
                            <Row>
                                <Col lg={6}>
                                    <Card>
                                        <Card.Header>
                                            <h6 className="mb-0">📊 {symbol} vs {benchmarkSymbol}</h6>
                                        </Card.Header>
                                        <Card.Body>
                                            <div className="text-center mb-3">
                                                ...
                                                <p className="text-muted">
                                                    {symbol} is {marketComparison.outperforming ? 'outperforming' : 'underperforming'} {benchmarkSymbol} today
                                                </p>
                                            </div>
                                        </Card.Body>
                                    </Card>
                                ...
                                <Col lg={6}>
                                    <Card>
                                        ...
                                        <Card.Body>
                                            <p>Detailed comparison analysis with {benchmarkSymbol} benchmark will be displayed here.</p>
                                        </Card.Body>
                                    </Card>
                                </Col>
                            </Row>
                            ...
                            <Card>
                                <Card.Body className="text-center">
                                    {loadingData ? <Spinner animation="border" /> : 'Market comparison data not available'}
                                </Card.Body>
                            </Card>
                        )}
                    </Tab.Pane>
