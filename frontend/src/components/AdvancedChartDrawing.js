...
export default function AdvancedChartDrawing({ data, patterns, onDrawingComplete }) {
  ...
  const drawPatterns = (ctx, width, height) => {
    ...
    if (patterns.trend_lines) {
      ...
      ctx.strokeStyle =
        trend === 'UPTREND' ? '#4CAF50' : trend === 'DOWNTREND' ? '#F44336' : '#FFC107';
      ctx.lineWidth = 2;
      ctx.beginPath();
    ...
    <Card>
      <Card.Header className='d-flex justify-content-between align-items-center'>
        ...
        <div>
          ...
          <ButtonGroup>
            <Button variant='outline-secondary' size='sm' onClick={() => setDrawingMode('none')}>
              🖱️ Select
            </Button>
            <Button variant='outline-danger' size='sm' onClick={clearDrawings}>
              🗑️ Clear
            </Button>
          </ButtonGroup>
        </div>
      ...
      <Card.Body>
        ...
        <div className='mt-3'>
          <small className='text-muted'>
            🎨 Drawing Mode: <strong>{drawingMode}</strong> | 📊 Patterns Detected:{' '}
            <strong>{patterns?.chart_patterns?.length || 0}</strong> | ✏️ User Drawings:{' '}
